...
new_data = data%>% dplyr::select(n_rooms, n_bathrooms, sq_mt_built, buy_price_by_area, buy_price)
new_data = na.omit(new_data)

set.seed(123)
idx=createDataPartition(new_data$buy_price, p = 0.75, list = FALSE)  

train_data = new_data[idx,]
test_data = new_data[-idx,]


model <-  lm(log(buy_price) ~ n_rooms + n_bathrooms + log(sq_mt_built) + log(buy_price_by_area), data=new_data)
summary(model)
predicted <- predict(model, newdata = test_data)
error <- sqrt(mean((test_data$buy_price - predicted)^2))

# Check for overfitting
train_predicted <- predict(model, newdata = train_data)
train_error <- sqrt(mean((train_data$buy_price - train_predicted)^2))
...
```{r}
colSums(is.na(data))
data$is_new_development = NULL
data$orientation = as.factor(data$orientation)
data$neighborhood_id = as.factor(data$neighborhood_id)
new_data  <- subset(data, select = -c(n_floors, sq_mt_allotment,parking_price,subtitle, id , title , street_name, street_number, lat, long ))
new_data = na.omit(new_data)
idx=createDataPartition(new_data$buy_price, p = 0.75, list = FALSE)  

training = new_data[idx,]
testing = new_data[-idx,]
nrow(training) #75%
nrow(testing) 
...
# Train Lasso model using CV
lasso.model <- train(buy_price ~ ., data = training, method = "glmnet", trControl = ctrl, tuneGrid = expand.grid(alpha = 1, lambda =opt.lambda ))

# Make predictions on the testing set
y.pred <- predict(lasso.model, newdata = testing)

# Calculate the accuracy of the predictions
accuracy <- cor(y.pred, testing$buy_price) ** 2
accuracy

best_model <- glmnet(x, y, alpha = 1, lambda = opt.lambda)
#coef(best_model)
...
# Train Ridge model using CV
ridge.model <- train(buy_price ~ ., data = training, method = "glmnet", trControl = ctrl, tuneGrid = expand.grid(alpha = 0, lambda =opt.lambda ))

# Make predictions on the testing set
y.pred <- predict(ridge.model, newdata = testing)

# Calculate the accuracy of the predictions
accuracy <- cor(y.pred, testing$buy_price) ** 2
accuracy

best_model <- glmnet(x, y, alpha = 0, lambda = opt.lambda)
#coef(best_model)
...
# Train Ridge model using CV
en.model=train(buy_price ~ ., data = training, method = "glmnet", trControl = ctrl, tuneGrid = expand.grid(alpha = model$bestTune$alpha, lambda =opt.lambda ))

# Make predictions on the testing set
y.pred <- predict(en.model, newdata = testing)

# Calculate the accuracy of the predictions
accuracy <- cor(y.pred, testing$buy_price) ** 2
accuracy

best_model <- glmnet(x, y, alpha = model$bestTune$alpha, lambda = opt.lambda)
enet_coefs = coef(best_model)
```
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      numericInput("n_rooms", "number_of_rooms:", value = 3, min = 0, max = 10, step = 1)
    ),
    mainPanel(
      plotOutput("plot")
    )
...
server <- function(input, output) {
  # Crea una función reactiva que utilice el modelo
  predicted_price <- reactive({
    numrooms <- input$n_rooms
    data.frame(n_rooms = numrooms) %>%
      predict(mod_both)
  })
  
  # Genera una salida para visualizar los resultados del modelo
  output$plot <- renderPlot({
    numrooms <- input$n_rooms
    predicted <- predicted_price()
    
    ggplot(new_data, aes(x = n_rooms, y = buy_price)) +
      geom_point() +
      geom_smooth(method = "lm") +
      geom_vline(xintercept = numrooms, linetype = "dashed", color = "red") +
      geom_hline(yintercept = predicted, linetype = "dashed", color = "blue") +
      labs(, x = "Number of rooms", y = "Price of house")
  })
}
